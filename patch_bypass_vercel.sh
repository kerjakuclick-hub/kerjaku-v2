      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
          ...
          <span className="text-xs font-bold bg-blue-100 text-blue-700 px-3 py-1 rounded-full border border-blue-200 shadow-sm">
            Mendeteksi {daftarKsatria.length} Ksatria di Database
          </span>
        </div>
                  ...
                  <tr key={order.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-4 text-right">
                      {order.status === 'PENDING' ? (
                        ...
                        /* INILAH TAKTIK BYPASS-NYA: Kita bungkus pemanggilan form dengan fungsi asinkron kosong */
                        <form action={async (formData) => {
                          'use server';
                          await assignKsatria(formData);
                        }} className="flex flex-wrap items-center gap-2 justify-end">
                          
                          <input type="hidden" name="order_id" value={order.id} />
                          {daftarKsatria.length === 0 ? (
                             <span className="text-xs font-bold text-red-500 bg-red-50 px-3 py-1.5 rounded-lg border border-red-200">
                               Database tidak merespons Ksatria
                             </span>
                          ) : (
                            <>
                              <select 
                                name="ksatria_id" 
                                required 
                                defaultValue={daftarKsatria.length === 1 ? daftarKsatria[0].id : ""}
                                className="p-2.5 text-xs border border-slate-300 rounded-lg outline-none bg-white text-slate-900 focus:border-blue-600 font-bold cursor-pointer min-w-[150px] shadow-sm"
                              >
                                {daftarKsatria.length !== 1 && <option value="">-- Pilih Ksatria --</option>}
                                {daftarKsatria.map((k: any) => (
                                  <option key={k.id} value={k.id}>{k.full_name || 'Tanpa Nama'} ({k.phone_number || '-'})</option>
                                ))}
                              </select>
